...
CREATE TABLE employees(
    ...
    name                 VARCHAR2(20),
    surname              VARCHAR2(20),
    service_id  	NUMBER(4) NOT NULL CONSTRAINT employees_services_fk REFERENCES services(service_id) ON DELETE CASCADE,
    hotel_id        NUMBER(4)
);
...
CREATE TABLE hotel(
    ...
    address_id  NUMBER(4) NOT NULL CONSTRAINT hotel_address_fk REFERENCES address(address_id) ON DELETE CASCADE,
    sales_id      NUMBER(4) CONSTRAINT hotel_sales_fk REFERENCES sales(sales_id) ON DELETE SET NULL
);
...
CREATE TABLE rooms(
    ...
    room_size        NUMBER(4),
    price            NUMBER(4),
    type_id     NUMBER(4) CONSTRAINT rooms_type_fk REFERENCES roomtype(type_id) ON DELETE SET NULL,
    floor_id  NUMBER(4) NOT NULL CONSTRAINT rooms_floors_fk REFERENCES floors(floor_id) ON DELETE CASCADE
);

CREATE TABLE equipment(
    equipment_id          NUMBER(4) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 100 CONSTRAINT equipment_pk PRIMARY KEY,
    room_id           NUMBER(4) NOT NULL CONSTRAINT equipment_rooms_fk REFERENCES rooms(room_id) ON DELETE CASCADE,
    furniture_id  NUMBER(4) NOT NULL CONSTRAINT equipment_furniture_fk REFERENCES futniture(furniture_id) ON DELETE CASCADE
);
...
CREATE TABLE reservations(
    ...
    price           NUMBER(8),
    room_id   NUMBER(4) CONSTRAINT reservations_rooms_fk REFERENCES rooms(room_id) ON DELETE SET NULL,
    guest_id NUMBER(4) CONSTRAINT reservations_guests_fk REFERENCES guests(guest_id) ON DELETE SET NULL,
    guests_number   NUMBER(4)
);

---CONSTRAINTS

ALTER TABLE address ADD CONSTRAINT address_hotel_fk FOREIGN KEY(hotel_id) REFERENCES hotel(hotel_id) ON DELETE SET NULL;
ALTER TABLE sales ADD CONSTRAINT sales_hotel_fk FOREIGN KEY(hotel_id) REFERENCES hotel(hotel_id) ON DELETE SET NULL;
ALTER TABLE employees ADD CONSTRAINT employees_hotel_fk FOREIGN KEY(hotel_id) REFERENCES hotel(hotel_id) ON DELETE CASCADE;
ALTER TABLE floors ADD CONSTRAINT floors_hotel_fk FOREIGN KEY(hotel_id) REFERENCES hotel(hotel_id) ON DELETE CASCADE;
